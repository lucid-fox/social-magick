...
class FileDistributor
{
	...
	public static function ensureDistributed(string $basePath, string $fileName, int $levels = 1): string
	{
		...
		// If it's a forcibly flat directory just return the flat path.
		if ($levels === 0)
		{
			return $flatPath;
		}
		...
		while (!empty($relPath))
		{
			...
			$didMove = @rename($sourceFile, $distributedPath);

			if (!$didMove)
			{
				$didMove = File::move($sourceFile, $distributedPath);
			}
		...
		if (@file_exists($distributedPath))
		{
		...
		for ($newLevel = $levels + 1; $newLevel <= 5; $newLevel++)
		{
			...
			// Try to move the file from the legacy directory to the distributed path.
			$didMove = @rename($sourcePath, $distributedPath);

			if (!$didMove)
			{
				$didMove = File::move($sourcePath, $distributedPath);
			}

			return $didMove ? $distributedPath : $sourcePath;
		}
